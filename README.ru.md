...
**В начале сессии подставляет то, что знает Honcho**: вашу peer card (карточку с ключевыми фактами о вас), сводку сессии этого
...
| (комментарий в корне)    | `sessions` можно также задать в корне как запасной вариант; блок host имеет приоритет |
...
- **`dialectic`** (обоснованный ответ Honcho на основе всего, что он знает о вас) — запускается каждые `cadence.dialectic` ходов, форма задаётся
  `injection.dialectic`. После первого хода ничто его не ждёт, поэтому запоздавший ответ попадёт в следующий ход.

Компоненты, которые есть в canonical schema (канонической схеме конфигурации), но не реализованы в этом плагине —
...
детальный `observation`, `multiUser`), и ключей, переименованных в одной из предыдущих версий. Ни один
параметр не будет принят, а затем молча проигнорирован.
...
любого пути можно закрепить другое имя через корневую карту `sessions` — переопределение всегда имеет приоритет.
...
| Команда          | Действие                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------ |
| `/honcho`        | Статус: peer, workspace, сессия, стратегия, ожидающие отправки, последняя синхронизация, последнее получение данных |
...
Контракт `~/.honcho/config.json`, соглашение об именовании сессий и `src/redact.ts` взяты из родственных
